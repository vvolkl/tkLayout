...
bool AnalyzerMatBudget::analyze()
{
  ...
  for (auto iTracker : m_trackers) {
    ...
    for (auto iTrack=0; iTrack<m_nTracks; iTrack++) {
      ...
      //
      // Get material related to beam-pipe, detector modules (so-called module caps) etc. through material visitor
      ...
      // Study beam pipe -> fill only once (to avoid multiplicative fill for each tracker)
      if (iTracker==*m_trackers.begin()) {

        m_radMB["Beampipe"]["Total"].Fill(eta, matBudget["Beampipe"].radiation);
        m_intMB["Beampipe"]["Total"].Fill(eta, matBudget["Beampipe"].interaction);

        m_radMB["Tracker"]["Total"].Fill(eta, matBudget["Beampipe"].radiation);
        m_intMB["Tracker"]["Total"].Fill(eta, matBudget["Beampipe"].interaction);
      }
      ...
      m_intMB[trkName]["Total"].Fill(eta, matBudget["Barrel"].interaction +
                                          ...
                                          matBudget["BeamPipe"].interaction);
